...
lemma bounded_above_of_bounded_set (h : bounded_set S) : bounded_above S := by
  obtain ⟨M,hM⟩ := h
  use M; intro x hx
  exact le_of_lt (lt_of_abs_lt (hM x hx))

lemma bounded_below_of_bounded_set (h : bounded_set S) : bounded_below S := by
  obtain ⟨M,hM⟩ := h
  use -M; intro x hx
  apply le_of_lt
  specialize hM x hx
  apply neg_lt_of_abs_lt at hM; exact hM
...
theorem intermediate_value {f : ℝ → ℝ} {a b y: ℝ} (h : sq_continuous_on f (Icc a b)) (hy : y ∈ Icc (f a) (f b)) (alb : a < b) :
  ...
  set S : Set ℝ := {x : ℝ | x ∈ Icc a b ∧ f x ≤ y} with hS
  have neS : ∃ x, x ∈ S := by use a; constructor; exact ⟨le_refl a, le_of_lt alb⟩; exact le_of_lt four
  have bS : bounded_above S := by use b; intro x xS; exact xS.1.2

  obtain ⟨c, supc⟩ := exists_sup bS neS
  unfold sup at supc
  use c; constructor
  constructor;
  unfold UpBound at supc
  apply supc.1
  refine Set.mem_sep ?_ fay; constructor
  exact le_refl a
  exact le_of_lt alb
  apply supc.2 b
  intro x xS
  ...
  have ledger : f c ≥ y := by
    ...
    have mlem : ∀ n, f (g n) ≥ y := by
      intro n
      have gab : g n ∈ Icc a b := by
        unfold g; constructor
        apply le_min; trans c; apply supc.1
        refine Set.mem_sep ?_ fay; constructor
        exact le_refl a; exact le_of_lt alb
    ...
    -- have lemur : ∀ x ∈ S, f x ≤ y := by intro x xS; apply le_of_lt; exact xS.2
    -- by_contra! hidle
    -- have : ∃ x ∈ S, f x > y := by
    --   use c; constructor;
  ...
  -- let rec a' n : ℝ := match n with
  --   | 0 => a
  --   | n + 1 => if f (a' n) < y then



def bounded_on (f : ℝ → ℝ) (D : Set ℝ) :=
  ∃ M, ∀ x ∈ D, |f x| < M

--Prop 12.4
theorem bounded_of_cont_of_compact (h : continuous_on f Q) (hb : compact Q) (nempty : ∃ x, x ∈ Q) : bounded_on f Q := by
  unfold bounded_on
  ...
  use a; constructor
  intro n; rw[ha]; dsimp
  apply (Classical.choose_spec (h n)).1
  constructor
  intro n hn; rw[ha]; dsimp; use 0
  intro m mnn
  sorry
  intro h'
  apply converges_of_convto at h'
  apply bounded_of_converges at h'
  unfold bounded boundedby at h'
  contrapose! h'
  intro M; specialize h M
  rw[ha]; dsimp

--Thm 12.3
theorem extreme_value {f : ℝ → ℝ} (h : continuous_on f Q) (kQ : compact Q) (nempty : ∃ x, x ∈ Q) :
  ∃ c ∈ Q, ∀ x ∈ Q, f c ≥ f x := by
  have bfQ : bounded_on f Q := bounded_of_cont_of_compact h kQ nempty
  have bpf : bounded_set (f '' Q) := by
    obtain ⟨M,hM⟩ := bfQ
    use M; intro y hy
    obtain ⟨x,xQ⟩ := hy
    rw[← xQ.2]; exact hM x xQ.1

  have gg : ∃ x, x ∈ f '' Q := by
    obtain ⟨x,xQ⟩ := nempty
    use f x; exact Set.mem_image_of_mem f xQ
  obtain ⟨M,hM⟩ := exists_sup (bounded_above_of_bounded_set bpf) gg




--Thm 12.4
theorem compact_image_of_cont_of_compact (f : ℝ → ℝ) (Q : Set ℝ) (h : continuous_on f Q) (kQ : compact Q) :
  compact (f '' Q) := by
  unfold compact
  intro a ainQ
  have hb : ∃ (b : ℕ → ℝ), (in_set b Q) ∧ (a = fun n ↦ f (b n)) := by
    have : ∀ n, ∃ y ∈ Q, a n = f y := by
      intro n
      unfold in_set at ainQ; specialize ainQ n
      obtain ⟨y,yQ,ay⟩ := ainQ
      use y; exact ⟨yQ,by rw[ay]⟩
    choose b hb using this
    use b; constructor
    intro n; exact (hb n).1
    funext n; exact (hb n).2
  obtain ⟨b, ⟨bQ,ab⟩⟩ := hb
  obtain ⟨L,LQ,hc⟩ := kQ b bQ
  obtain ⟨c,cb,cc⟩ := hc
  use f L; constructor
  simp; use L;
  unfold lim_pt
  use (fun n ↦ f (c n)); constructor
  obtain ⟨w, hw⟩ := cb
  rw[ab]; use w; intro k
  constructor; exact (hw k).1
  dsimp; rw[(hw k).2]
  rw[← continuous_eq'] at h
  apply h; exact LQ
  intro n; obtain ⟨w,hw⟩ := cb
  rw[(hw n).2]; exact bQ (w n)
  exact cc
